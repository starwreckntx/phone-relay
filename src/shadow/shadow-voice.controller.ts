 ...
 * both agents.
 *
 * Because caller ID is spoofable, an optional DTMF PIN (SHADOW_PIN) gates entry:
...
export class ShadowVoiceController {
  ...
  private get voice(): string {
    return this.config.get<string>('shadow.voice') || 'Google.en-US-Studio-O';
  }
  ...
  /** Digits-only PIN, or '' when no PIN challenge is configured. */
  private get pin(): string {
    return (this.config.get<string>('shadow.pin') || '').replace(/\D/g, '');
  }
  ...
  private gatherPin(twiml: Twilio.twiml.VoiceResponse, prompt: string) {
    const gather = twiml.gather({
      input: ['dtmf'],
      action: '/voice/shadow/verify',
      method: 'POST',
      numDigits: this.pin.length,
      timeout: 8,
      finishOnKey: '#',
      actionOnEmptyResult: true,
    } as any);
    gather.say({ voice: this.voice as any }, prompt);
    return gather;
  }
  ...
  async incoming(@Body() body: any, @Res() res: Response): Promise<void> {
    ...
    if (!this.shadow.isConfigured()) {
      ...
      // Second factor: caller ID got them here, but they must key the PIN.
      this.gatherPin(twiml, 'Enter your access code.');
    } else {
  ...
  @Post('verify')
  @HttpCode(HttpStatus.OK)
  async verify(@Body() body: any, @Res() res: Response): Promise<void> {
    const { CallSid, Digits } = body || {};
    const entered = String(Digits || '').replace(/\D/g, '');
    const twiml = new Twilio.twiml.VoiceResponse();

    if (this.pin && entered === this.pin) {
      this.gatherSpeech(twiml, this.greeting);
    } else {
      this.logger.warn('Shadow PIN rejected', {
        callSid: CallSid,
        event: 'shadow-pin-reject',
      });
      twiml.say(
        { voice: this.voice as any },
        'That code is not right. Goodbye.',
      );
      twiml.hangup();
      if (CallSid) this.shadow.reset(CallSid);
    }
  ...
  async reply(@Body() body: any, @Res() res: Response): Promise<void> {
    ...
    if (this.shadow.turnsFor(CallSid) >= this.maxTurns) {
      twiml.say(
        { voice: this.voice as any },
        "That's a long session. Let's pick it up later. Goodbye.",
      );
      twiml.hangup();
      if (CallSid) this.shadow.reset(CallSid);
    } else {
